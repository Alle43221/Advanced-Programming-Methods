...
public class ServiceCommunities {
    ...
    private void setAdjList(){
        adjList = new HashMap<>();
        network.getUserRepo().findAll().forEach(
                user->{
                    List<Long> friends = new ArrayList<>();
                    StreamSupport.stream(network.getFriendshipRepo().findAll().spliterator(), false).forEach(
                            friendship->{
...
//        }
    ...
    public Long connectedCommunities() {
        ...
        StreamSupport.stream(network.getUserRepo().findAll().spliterator(), false).forEach(
                user->{
                    ids.add(user.getId());
                }
        );
    ...
    public List<Long> mostSocialCommunity() {
        ...
        // list of ids of users
        List<Long> ids = new ArrayList<>();
        network.getUserRepo().findAll().forEach(user->{
            ids.add(user.getId());
                }
                );
        HashMap<Long, Boolean> visited = new HashMap<>();
