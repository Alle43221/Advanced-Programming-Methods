...
public class MovieController implements Observer<EntityChangeEvent> {
    ...
    private void initModel() {
        ...
        Page<Movie> movieList = movieService.findAllOnPage(new Pageable(currentPage, currentPageSize), filter);
        List<Movie> movies=StreamSupport.stream(movieList.elementsOnPage().spliterator(), false).toList();
        model.setAll(movies);
        int noOfPages=(int)Math.ceil((double)movieList.getTotalNumberOfElements()/currentPageSize);
        pageNumber.setText("Page:"+ (currentPage+1) +"/"+noOfPages);
        if(currentPage==0) previousButton.setDisable(true);
        if(currentPage+1>noOfPages) nextButton.setDisable(true);
    }
